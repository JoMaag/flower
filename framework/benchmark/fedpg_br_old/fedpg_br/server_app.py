...
class FedPGStrategy(Strategy):
    ...
    def configure_fit(self, server_round: int, parameters: Parameters, client_manager):
        ...
        # Adaptive batch size for FedPG-BR
    ...
    def _scsg_step(self, theta_n: torch.Tensor, theta_0: torch.Tensor, mu_t: torch.Tensor):
        policy_n = create_policy(self.state_dim, self.action_dim, self.config.env_name,
                                  self.config.hidden_units, self.config.activation, self.config.output_activation)
        policy_n.set_flat_params(theta_n)
        
        policy_0 = create_policy(self.state_dim, self.action_dim, self.config.env_name,
                                  self.config.hidden_units, self.config.activation, self.config.output_activation)
...
def server_fn(context: Context):
    ...
    use_fedpg_br = run_config.get("use-fedpg-br", False)
