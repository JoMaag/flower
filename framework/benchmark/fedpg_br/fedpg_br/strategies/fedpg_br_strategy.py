...
from fedpg_br.strategies.base import AggregationStrategy, register_strategy, apply_gradient
...
class FedPGBR(AggregationStrategy):
    ...
    def aggregate(self, gradients: List[torch.Tensor], batch_size: int, **kwargs) -> Tuple[torch.Tensor, List[int]]:
        ...
        if byzantine_filter is None:
            raise ValueError("FedPG-BR requires a byzantine_filter in kwargs")
        return byzantine_filter.aggregate(gradients, batch_size)
    ...
    def server_update(self, policy, optimizer, theta_t_0, mu_t, config, env=None, env_name="") -> int:
        ...
        if env is None:
            raise ValueError("FedPG-BR server_update requires an env for SCSG sampling")
